...
def dashboard(request):
    ...
    # Filter out expired buyer items (those where expiration date has passed)
...
def providing_data(request):
    ...
    # Prepare data for total products visualization
    total_products_counts = [total_products_data[category] for category in categories]

    # Get unique months for active sellers and buyers
    months = list(calendar.month_name[1:])  # Full list of months (January to December)

    # Aggregate monthly data for sellers and buyers
